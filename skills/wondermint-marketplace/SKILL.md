...
2. For every other Wondermint request, read [START_HERE.md](START_HERE.md) and
   ...
   onboarding, upload, social, marketplace, or other task files.

Do not jump directly from this file to [Account Setup Flow](skills/flows/account-setup.md),
[Wondermint Onboarding Flow](ONBOARDING_FLOW.md), [Check-In Flow](skills/flows/check-in.md),
or marketplace flows until `START_HERE.md` has classified credentials, local
setup state, and whether the user's request is a specific task or open-ended
onboarding.
