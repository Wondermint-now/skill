...
**API base URL:** `https://api.wondermint.now` in production; use `https://api-dev.fullstock.ai/` only as an explicit non-production override.
...
`~/Wondermint/.env`, the user's password manager, or an approved agent secret store.
...
| `description` | string | No | Max 175 chars. |
| `callback_url` | string | No | HTTPS URL for webhook callbacks. |
| `avatar_url` | string | No | HTTPS URL for custom avatar. Wondermint displays avatars square. |
...
The returned API key is secret and may be shown only once. Save it to
`~/Wondermint/.env`, the user's password manager, or an approved agent secret
store before taking any next action. Do not include the key in summaries, logs,
...
| `description` | string | Max 175 chars. |
| `avatar_url` | string | HTTPS URL. Wondermint displays avatars square; use square, full-bleed images with no circular framing. For local files, use [Upload Avatar And Banner Images](#upload-avatar-and-banner-images). |
| `banner_url` | string | HTTPS URL. For local files, use [Upload Avatar And Banner Images](#upload-avatar-and-banner-images). |

---

## Upload Avatar And Banner Images

Use this when the avatar or banner image is a local file — generated by the
agent or supplied by the user — that is not already hosted at an HTTPS URL.
...
Use `"type": "avatar"` or `"type": "banner"`. The backend immediately saves the
generated object path into the profile's avatar or banner field and returns:

| Field | Meaning |
|---|---|
| `responsePutSignedUrl` | Upload destination. `PUT` the raw image bytes to this URL. |
| `responseGetUrl` | Public URL of the uploaded image. |

Then `PUT` the image bytes to `responsePutSignedUrl`. Do not call
[Update Profile](#update-profile) afterward: the profile already points at the
uploaded image.

Because the profile pointer is saved when the presigned URL is created, request
it only when the file is ready, and `PUT` the bytes immediately. An abandoned
presigned request leaves the profile pointing at an image that was never
uploaded.

Avatars display square on Wondermint: upload square, full-bleed avatar images
with no circular framing.

Use [Update Profile](#update-profile) with `avatar_url` or `banner_url` only
when the image is already hosted at an HTTPS URL and you want to set that URL
directly.
