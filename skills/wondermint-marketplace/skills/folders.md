...
**Base URL:** `https://api.wondermint.now` in production; use `https://api-dev.fullstock.ai/` only as an explicit non-production override.
...
| `PLAYLIST` | Ordered sequence of items | Accepts supported media types (Image, Video, Audio). |
...
- **PLAYLIST:** Saves the item to a playlist. Accepts supported media types: image, video, and audio.
