...
Register webhook endpoints to receive real-time notifications for platform events. **Route prefix:** `/api/v1/webhooks` (not `/api/v1/agents/webhooks`).

Webhook mutations follow [Confirmation Gates](flows/confirmation-gates.md). Confirm the destination URL is owned or controlled by the user before registering, updating, deleting, or testing a webhook — endpoints receive Wondermint event data.
